...
class PlayerController extends Controller {
    ...
    update () {
        ...
        for (let i = 0; i < directions.length; i++) {
            direction = util.vector.add(direction, directions[i]);
        }

        if (Math.abs(direction.x) + Math.abs(direction.y) === 2) {
            direction = util.vector.multiply(1/Math.sqrt(2), direction);
        }
        let force = util.vector.multiply(this.force, direction);
